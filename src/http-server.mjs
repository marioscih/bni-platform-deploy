  ...
  return createServer(async (request, response) => {
    ...
    try {
      ...
      if ((method === "GET" || method === "POST") && path.startsWith("/v2/admin/")) {
        ...
        if (path === "/v2/admin/migrate-account-history") return json(response, 200, { data: await platform.provisioning.migrateAccountHistory(input) }, correlationId);
        if (path === "/v2/admin/migrate-card-profiles") return json(response, 200, { data: await platform.provisioning.migrateCardProfiles(input) }, correlationId);
        if (path === "/v2/admin/correct-account-holder") return json(response, 200, { data: await platform.provisioning.correctAccountHolder(input) }, correlationId);
      ...
      if (method === "POST" && path === "/v2/mobile/login/bootstrap") {
        const input = await body(request);
        const login = await platform.identity.completeLogin(input);
        const tokens = await platform.identity.exchangeAuthorizationCode({ authorizationCode: login.authorizationCode });
        const principal = platform.identity.authenticate(tokens.accessToken);
        const accounts = platform.ledger.accountsForOwner(principal.sub);
        const movements = accounts.length ? platform.ledger.movements(accounts[0].accountReference, principal.sub, { limit: 50 }) : { items: [], nextCursor: null };
        return json(response, 200, { data: { ...tokens, accounts, movements, cards: platform.cards.list(principal.sub), capabilities: platform.capabilities() } }, correlationId);
      }
      ...
      const movementDetail = path.match(/^\/v2\/accounts\/([^/]+)\/movements\/([^/]+)$/);
      if (method === "GET" && movementDetail) return json(response, 200, { data: platform.ledger.movement(movementDetail[1], movementDetail[2], principal.sub) }, correlationId);
      ...
      if (method === "POST" && path === "/v2/transfers/prepare") { const input = await body(request); return json(response, 201, { data: await platform.transfers.prepare({ ...input, customerReference: principal.sub, idempotencyKey: requiredIdempotency(request, input) }) }, correlationId); }
      ...
      const cardControlSca = path.match(/^\/v2\/cards\/([^/]+)\/controls\/sca$/); if (method === "POST" && cardControlSca) { const input = await body(request); return json(response, 200, { data: await platform.cards.createControlChallenge({ ...input, cardReference: cardControlSca[1], ownerReference: principal.sub, deviceReference: principal.device, idempotencyKey: requiredIdempotency(request, input) }) }, correlationId); }
      const cardControl = path.match(/^\/v2\/cards\/([^/]+)\/controls$/); if (method === "POST" && cardControl) { const input = await body(request); return json(response, 200, { data: await platform.cards.setControlsWithSca({ ...input, cardReference: cardControl[1], ownerReference: principal.sub, idempotencyKey: requiredIdempotency(request, input) }) }, correlationId); }
      ...
      const walletChallenge = path.match(/^\/v2\/cards\/([^/]+)\/wallet\/sca$/); if (method === "POST" && walletChallenge) { const input = await body(request); return json(response, 200, { data: await platform.cards.createWalletChallenge({ ...input, cardReference: walletChallenge[1], ownerReference: principal.sub, deviceReference: principal.device, idempotencyKey: requiredIdempotency(request, input) }) }, correlationId); }
      const walletProvision = path.match(/^\/v2\/cards\/([^/]+)\/wallet$/); if (method === "POST" && walletProvision) { const input = await body(request); return json(response, 200, { data: await platform.cards.provisionWalletWithSca({ ...input, cardReference: walletProvision[1], ownerReference: principal.sub, deviceReference: principal.device, idempotencyKey: requiredIdempotency(request, input) }) }, correlationId); }
      ...
      const bniDecline = path.match(/^\/v2\/bni-pay\/payment-intents\/([^/]+)\/decline$/); if (method === "POST" && bniDecline) { const input = await body(request); return json(response, 200, { data: await platform.bniPay.declineByCustomer({ ...input, paymentIntentId: bniDecline[1], customerReference: principal.sub, idempotencyKey: requiredIdempotency(request, input) }) }, correlationId); }
      throw new BankError("NOT_FOUND", 404);
    } catch (error) {
